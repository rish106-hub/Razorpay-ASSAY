...
def test_adverse_event_rejects_generic_twenty_one_character_identifier() -> None:
    source_frame = pl.DataFrame(
        {
            "Order Date": [date(2026, 8, 21)],
            "Order Particulars": ["Regulatory order"],
            "Entity / Individual Name": ["Merchant Evidence"],
            "PAN": [""],
            "DIN / CIN": ["123456789012345678901"],
            "Symbol ": [""],
            "Period": [""],
            "NSE Circular No. (For Debarment)": [""],
            "Date of NSE circular": [None],
            "NSE Circular No. (For Revocation)": [""],
            "Date of NSE circular. (For Revocation)": [None],
            "source_row_number": [2],
        }
    ...
    assert adverse_frame["cin_candidate"].item() is None
